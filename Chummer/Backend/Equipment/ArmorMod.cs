...
namespace Chummer.Backend.Equipment
{
    ...
    {
        private static readonly ILogger<ArmorMod> Log = TemporaryServiceLocator.Services
            .GetRequiredService<ILogger<ArmorMod>>();
        private Guid _guiID;
        ...
        private async Task CreateCoreAsync(bool blnSync, XmlNode objXmlArmorNode, int intRating, IList<Weapon> lstWeapons, bool blnSkipCost = false, bool blnSkipSelectForms = false, CancellationToken token = default)
        {
            ...
            {
                Log.Warn("Missing id field for armor mod xmlnode " + objXmlArmorNode.ToString());
                Utils.BreakIfDebug();
            }
            else
            {
